...
export const Inpusts = ({ type, text, onChange, id, validate }) => {
  const [showPassword, setShowPassword] = useState(false);
  return (
    <div className="w-full flex border-2 border-gray-100 rounded-xl bg-transparent">
      <input
...
export const Buttons = ({ text }) => {
  return (
    <button className=" disabled:bg-blue-100 disabled:cursor-not-allowed  active:scale-[.98] active:duration-75 transition-all hover:scale-[1.01] ease-in-out py-3 rounded-xl bg-blue-600 text-white text-lg font-bold w-[100%] mt-5">
      {text}
    </button>
  );
};
...
export const Checkboxes = ({ text, id }) => {
  return (
